...
g_NMDAR,input_pattern,n_inputs = [65, 'rdp', 3] 
n_RyR, tau_refill, f_input = sys.argv[1].split('_')
n_RyR = int(n_RyR)
...
s = 1
t = np.arange(0., 300. ,1e-2)
CaMTimeCourse = np.zeros(t.shape[0])
xinit = sol[-1,:]
############################################### RDP Protocol ####################################
...
if n_RyR == 0:
    ...
    CaMTimeCourse[:] = [np.sum(sol[k,-23:-16]) for k in range(t.shape[0])]
else:
    sol = odeint(spine_model, xinit, t, atol=atol,rtol=rtol)
    CaMTimeCourse[:] = [np.sum(sol[k,-23:-16]) for k in range(t.shape[0])]
...
############################# saving into file #######################################################
fName = '/storage/vikrant/CaM_rdp/Cam_rdp' + str(f_input) + '_' + str(tau_refill) + '_' + str(n_RyR) + '.csv'

np.savetxt(fName, CaMTimeCourse, fmt = ['%f'], delimiter = ',', header = '', comments = '') 
